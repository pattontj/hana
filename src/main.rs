...
impl Func {
    fn bind_params(&mut self, args: Vec<Box<Form>>, env: &mut Environment) {
        ...
        for (param, arg) in zip(&self.params, args) {
            match param {
                Symbol(param) => self.context.bind_symbol(param.clone(), *arg),
                _ => {
...
*/
fn build_ast_from_form(pair: pest::iterators::Pair<Rule>) -> Form {
    match pair.as_rule() {
        Rule::integer => {
